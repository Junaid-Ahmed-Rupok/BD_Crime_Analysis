...
def predict():
    ...
    # Manual type conversion for specific columns (add this if automatic conversion fails)
    numeric_columns = [col for col in model_features if feature_info.get(col) == 'number']
    ...
    x = 'murder' if predictions == 0 else 'rape' if predictions == 1 else 'assault' if predictions == 2 else 'bodyfound' if predictions == 3 else 'kidnap' if predictions == 4 else 'robbery'
